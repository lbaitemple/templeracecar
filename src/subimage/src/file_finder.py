...
def get_iter_no(folder='data', filename='output_', extension='.c1sv'):
    iter_name = 0
...
def get_relative_filename(iter_name, folder='data', filename='output_', extension='.c2sv'):
    fileoutname = filename + str(iter_name) + extension
    fileoutname = os.path.join(folder, fileoutname)
    return fileoutname

# Get latest filename for outfile
def get_new_filename(folder='data', filename='output_', extension='.cs3v'):
...
def get_latest_filename(folder='data', filename='output_', extension='.c4sv'):
    check_folder_exists(folder)
